...
bool find_path_for_allwork(vector<query_input> &query,vector<edge_input>edge_input,int k) {
    ...
    for (int i = 0; i < query.size(); i++) {
        cout << "第" << i << "条业务" << endl;
        //auto l= k_short_path(query[i].u,query[i].v,k,query[i].k_short_path);
        f &= short_path(query[i].u,query[i].v,query[i].k_short_path,edge_input);
...
int main(){
    ...
    string pathin = "E:\\c++\\paper\\ConsoleApplication1\\crime-moreno6.in";
    string pathout = "E:\\c++\\paper\\ConsoleApplication1\\WOH_crime-moreno6.out";
    ...
    int k = 1;
    find_path_for_allwork(query_input, edge_input,k);
