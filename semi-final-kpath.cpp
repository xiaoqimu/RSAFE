...
int main(){
    ...
    //     vector<vector<int> >ans_edge;
    //     k_short_path(query_input[i].u,query_input[i].v,3,ans_edge,-1,1);

    //     cout << "第 " << i+1 << "条业务的情况\n";
    //     for(size_t j = 0;j < ans_edge.size();j++){
    //         cout << "第 " << j+1 << "条边：";
    //         for(size_t k = 0;k < ans_edge[j].size();k++){
    //             cout << e[ans_edge[j][k]].id << " ";
    //         }
    //         cout << "\n";
    //     }
