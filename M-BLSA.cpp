...
struct query_input{
    ...
    int targetpathindex;//在k_short_path中的位置
    query_input(){}
...
void forbid_edge(const vector<int>edge,int f){
    for(auto pos:edge){
        e[pos].used = f;
        if(pos&1){
            e[pos-1].used = f;
...
int k_short_path(int s,int t,int k,vector<vector<int> >&ans_edge,vector<edge_input>edge_input){
    ans_edge.clear();
    ...
    for(int i = 0;i < k;i++){
        ...
        if(f==0)break;
        forbid_edge(nowans,1);
        ans_edge.push_back(nowans);
    }
    for(int i = 0;i < ans_edge.size();i++){
        forbid_edge(ans_edge[i], 0);
    }

    return 1;
}

bool find_k_path_for_allwork(vector<query_input> &query,vector<edge_input>edge_input, int k) {
    bool f = 1;
    for (int i = 0; i < query.size(); i++) {
        cout << "第" << i << "条业务" << endl;
        f &= k_short_path(query[i].u,query[i].v,k,query[i].k_short_path,edge_input);
        //cout << query[i].k_short_path.size() << "\n";
    }
    return f;
}

void cal_targetpath(query_input &query) {
    assert((int)query.k_short_path.size() >= 1);
    int ansid = 0;
    int min_pnums = INT_MAX;
    for (int i = 0; i < query.k_short_path.size(); i++) {
        vector<int> path = query.k_short_path[i];
        int max_pnum = 0;
        for (int j = 0; j < path.size();j++){
            max_pnum = max(e[path[j]].p_nums, max_pnum);
        }
        if(max_pnum<min_pnums){
            min_pnums = max_pnum;
            ansid = i;
        }
    }
    query.targetpathindex = ansid;
    //更新经过线路的所有值
    for (int i = 0; i < query.k_short_path[ansid].size();i++){
        int pos = query.k_short_path[ansid][i];
        e[pos].p_nums += query.num;
        if(pos&1)e[pos-1].p_nums += query.num;
        else e[pos+1].p_nums += query.num;
    }
}
...
int main(){
    ...
    string pathout = "E:\\c++\\paper\\ConsoleApplication1\\MBLSA_email-univ6.out";
    readfile(pathin,pathout,edge_input, query_input);
    ...
    // }
    int k = 100;
    find_k_path_for_allwork(query_input, edge_input,k);
    //为每一个业务在其选择的k条最短路中找一条最适合的
    for (int i = 0; i < query_input.size();i++){
        cal_targetpath(query_input[i]);
    }
    vector<addedge> addedge;
    for (int i = 0; i < query_input.size();i++){
        //cout << "第 " << i << "条业务:"<<endl;
        allocation(query_input[i], addedge);
    }
    ...
    for(int i = 0;i < T;++i){
        ...
        work_ans.ans_edge_id = query_input[i].ans_edge_id;
        work_ans.p_id = query_input[i].p_id;
        ans[query_input[i].id] = work_ans;
    }

    //输出答案
    printanswer(pathout,addedge, ans, query_input);
