...
bool find_k_path_for_allwork(vector<query_input> &query, int k) {
    bool f = 1;
    for (int i = 0; i < query.size(); i++) {
        cout << "第" << i << "条业务" << endl;
        f &= k_short_path(query[i].u,query[i].v,k,query[i].k_short_path);
        assert(query[i].k_short_path.size()>0);
    }
    return f;
}
...
int main(){
    ...
    for(int j=6;j<=6;j++){
        string pathin = "E:\\c++\\paper\\ConsoleApplication1\\email-univ"+to_string(j)+".in";
        string pathout = "E:\\c++\\paper\\ConsoleApplication1\\BLSA_email-univ"+to_string(j)+".out";
        ...
        //cout << "读入完成" << endl;
        //cout << "读入完成0" << endl;
        find_k_path_for_allwork(query_input, k);
    // cout << "读入完成1" << endl;
    ...
    // cout << "读入完成3" << endl;
        ...
        ans_fe.push_back((int)addedge.size());
